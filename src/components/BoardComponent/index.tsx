import { Accessor, Component, For, Setter, createRenderEffect, createSignal, onMount } from "solid-js";
import NodeComponent from "../NodeComponent";
import ButtonsComponent from "../ButtonsComponent";
import styles from "./styles.module.css";
import EdgeComponent from "../EdgeComponent";
import NodeInputComponent from "../NodeInputComponent";
import { NodeRule } from "./Rules/NodeRule";
...
const BoardComponent: Component = () => {
    ...
    const [selectedNodeType, setSelectedNodeType] = createSignal<string | null>(null);
    ...
    const [nodesInput, setNodesInputs] = createSignal<NodeInput[]>([]);
    ...
    // Effects
    createRenderEffect(() => {
        setCode(handleGenerateCode());
    }, [nodes(), nodesInput(), edges()])
    ...
    function handleOnMouseUpBoard() {
        ...
        if (newEdge() !== null && insideInput() !== null) {
            ...
            switch(selectedNodeType()) {
                case "defaultNode":
                  nodeStart = nodes().find((node) => node.id === nodeStartId);
                  nodeEnd = nodes().find((node) => node.id === nodeEndId);
                  break;
                case "nodeInput":
                  nodeStart = nodesInput().find((node) => node.id === nodeStartId);
                  nodeEnd = nodesInput().find((node) => node.id === nodeEndId);
                  break;
            ...
            if (selectedNode() !== null) {
                ...
                switch(selectedNodeType()) {
                    case "defaultNode":
                      ...
                      break;
                    case "nodeInput":
                      node = nodesInput().find((node) => node.id === selectedNode());
                      break;
                    // Adicione mais casos conforme necessário
                    default:
                      node = null;
    ...
    function handleOnMouseDownNode(id: string, event: any, nodeType: string) {
        // Deselect edge
        setSelectedEdge(null);
    ...
    function handleOnClickAddInput(name: string, placeholder: string) {
        // Create random positions
        const randomX = Math.random() * window.innerWidth;
        const randomY = Math.random() * window.innerHeight;
        // Create signal
    ...
    function handleOnClickDelete() {
        ...
        for (let i = 0; i < uniqueEdges.length; i++) {
            ...
            if (edge) {
                ...
                switch(selectedNodeType()) {
                    case "defaultNode":
                      nodeStart = nodes().find((node) => node.id === edge.nodeStartId);
                      nodeEnd = nodes().find((node) => node.id === edge.nodeEndId);
                      break;
                    case "nodeInput":
                      nodeStart = nodesInput().find((node) => node.id === edge.nodeStartId);
                      nodeEnd = nodes().find((node) => node.id === edge.nodeEndId);
                      break;
                    // Adicione mais casos conforme necessário
                    default:
        ...
        switch(selectedNodeType()) {
            ...
            case "nodeInput":
              {
                setNodesInputs([...nodesInput().filter((node) => node.id !== selectedNode())]);
                setSelectedNode(null);
              }
              break;
            // Adicione mais casos conforme necessário
    ...
    function handleGenerateCode(): string {
        console.log(NodeRule(nodes(), edges()));
        
        let code = "";
        const generatedHTML = generateHTML(nodes(), edges());
        code+=generatedHTML;
        return code;
    }

    function generateHTML(nodes: Node[], edges: Edge[]): string {
        // const parentNodeMap: { [key: string]: Node[] } = {};
        // if(nodes.length > 0) {
        //     edges.forEach(edge => {
        //         if (!parentNodeMap[edge.nodeStartId]) {
        //             parentNodeMap[edge.nodeStartId] = [];
        //         }
        //         const node = nodes.find(node => node.id === edge.nodeEndId) as Node;
        //         parentNodeMap[edge.nodeStartId].push(node);
        //     });
        
        //     const buildXML = (nodeId: string, indentLevel: number): string => {
        //         let xml = "";
        //         const indent = "\t".repeat(indentLevel);
        //         if (parentNodeMap[nodeId]) {
        //             xml += `${indent}<node id="${nodeId}">\n`;
        //             parentNodeMap[nodeId].forEach(childNode => {
        //                 xml += buildXML(childNode.id, indentLevel + 1);
        //             });
        //             xml += `${indent}</node>\n`;
        //         } else {
        //             xml += `${indent}<node id="${nodeId}"/>\n`;
        ...
        //     let xmlScript = "<nodes>\n";
        //     xmlScript += buildXML(Object.keys(parentNodeMap)[0], 1);
        //     xmlScript += "</nodes>";
    ...
    return (
        <div id="boardWrapper" class={styles.wrapper}>
            <ButtonsComponent showDelete={selectedNode() !== null} onClickAdd={handleOnClickAdd} onCode={code()} onClickAddInput={handleOnClickAddInput} onClickDelete={handleOnClickDelete} />
            <div
                id="board"
